...
my $doc_head = 
 ("<HTML>\n" .
  ...
  "  <META NAME=\"Generator\"\n" .
  "    CONTENT=\"$progname $version, http://www.jwz.org/hacks/$progname\">\n".
  " </HEAD>\n" .
  " <BODY BGCOLOR=\"$bg_color1\" TEXT=\"$fg_color\"\n" .
...
#   if (m@^(<[^> \t\n]+>) (.*)$@s) {
  ...
  if ($head =~ m@^(\[|\*\*\*)@ ||
    ...
    $head = (($colorize_p && $bg_color ne $bg_color1) ? "<BR>" : "");
  } else {
    $head = "<B>$head</B>";
  }
  ...
  my $result = " <TR>" .
               "<TD VALIGN=TOP$color_style NOWRAP>$head</TD>" .
               "<TD VALIGN=TOP$color_style>$body</TD>";

  if ($stamp) {
    $result .= "<TD VALIGN=TOP$color_style ALIGN=RIGHT NOWRAP>$stamp</TD>";
  }
